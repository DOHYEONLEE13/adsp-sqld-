  ...
  steps: [
    {
      ...
      dialogue: [
        ...
        { pose: 'happy', text: '먼저 쉬운 역할 매칭을 풀고, 그다음 TRUNCATE처럼 헷갈리기 쉬운 명령을 볼 거야.' },
        { pose: 'happy', text: '[TRUNCATE]는 행을 비우지만 DDL이야. 그래서 실행되면 자동으로 COMMIT되고 ROLLBACK으로 되돌릴 수 없어.' },
        { pose: 'think', text: '"DROP은 DML인가?"처럼 이름만 보고 헷갈리는 보기가 나올 수 있어. DROP은 테이블 구조를 지우는 DDL이야.' },
        { pose: 'lightbulb', text: '"INSERT 가 DCL?" — X. INSERT = DML.' },
        { pose: 'idle', text: '이제 명령어가 어느 군에 속하는지 직접 골라보자.' },
      ],
      blocks: [
        {
          kind: 'intro',
        ...
        },
        {
          kind: 'callout',
          tone: 'warn',
          title: '헷갈리는 부분 1 — TRUNCATE 분류',
          body:
            'TRUNCATE는 행을 전부 비우기 때문에 DELETE처럼 느껴질 수 있습니다. 하지만 테이블을 다루는 DDL이라 자동 COMMIT되고 ROLLBACK으로 되돌릴 수 없습니다.',
        },
        {
          kind: 'callout',
          tone: 'warn',
          title: '헷갈리는 부분 2 — DROP/INSERT 분류',
          body:
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'lightbulb', text: '예: COALESCE(휴대폰, 집전화, 이메일) — 연락 할 수 있어한 첫 수단.' },
  ...
  steps: [
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'lightbulb', text: '③ [ON 조건]: 가장 [명시적]. 컬럼명이 [달라도] 쓸 수 있어.' },
    ...
    },
    {
      ...
      extraQuizIds: ['sqld-2-2-cp-04-subquery-place'],
      dialogue: [
        { pose: 'wave', text: '[서브쿼리(Subquery)]는 [쿼리 안의 또 다른 쿼리]. 자주 쓰는 도구.' },
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '윈도우 함수와 함께 자주 만나는 응용 영역입니다. TOP N은 DBMS별 문법 차이, PIVOT/UNPIVOT은 보고서 형태 변환, 계층형 질의는 조직도와 카테고리 구조를 떠올리면 이해하기 쉽습니다.',
        },
        {
          kind: 'table',
        ...
        },
        {
          ...
          body:
            'PIVOT: 행을 열로 펼침 (LONG → WIDE). 집계함수 꼭 알아둘 부분. FOR 컬럼 IN (값1 AS 별명1, 값2 AS 별명2). UNPIVOT: 열을 행으로 (WIDE → LONG). 집계 X.',
        },
        {
          kind: 'callout',
          tone: 'warn',
  ...
  steps: [
    {
      ...
      dialogue: [
        ...
        { pose: 'think', text: '[명시적 컬럼]: INSERT INTO T (col1, col3) VALUES (값1, 값3) — [선택 컬럼만 입력 할 수 있어].' },
        ...
        { pose: 'think', text: '헷갈리는 부분. [WHERE 누락 시] 모든 행이 변경. 무서운 실수.' },
        { pose: 'lightbulb', text: '[DELETE]: 행 삭제. DELETE FROM T WHERE 조건. WHERE 없으면 전체 행 삭제.' },
        { pose: 'happy', text: 'DELETE는 [DML이라 ROLLBACK 할 수 있어]. 반면 [TRUNCATE는 DDL이라 되돌릴 수 없어].' },
    ...
    },
    {
      ...
      dialogue: [
        { pose: 'wave', text: '[MERGE]는 [INSERT 와 UPDATE 를 하나로 묶은] 자주 쓰는 명령.' },
        ...
        { pose: 'happy', text: '한 번의 SQL 로 두 작업 처리 → [트랜잭션 안전성 높아짐], [성능 높아짐].' },
        { pose: 'think', text: 'ETL (데이터 통합), 증분 동기화, 매시간 갱신 작업에 표준 패턴.' },
        { pose: 'lightbulb', text: 'WHEN MATCHED 절에 [DELETE] 도 할 수 있어 (Oracle 일부 버전).' },
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'lightbulb', text: '[COMMIT]: 변경사항을 [영구 반영]. 이후에는 ROLLBACK으로 되돌릴 수 없어.' },
        ...
        { pose: 'happy', text: 'SAVEPOINT 문제는 SQL을 위에서 아래로 따라가며 마지막에 어떤 행이 남는지 확인하면 돼.' },
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            'TCL은 트랜잭션의 결과를 어떻게 처리할지 결정합니다. COMMIT은 확정, ROLLBACK은 되돌리기, SAVEPOINT는 중간 저장 지점이라고 보면 됩니다. SQL이 실행된 순서대로 현재 상태를 따라가면 됩니다.',
        },
        {
          kind: 'table',
          title: 'TCL 3종',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'think', text: 'DBMS 마다 동작이 [완전히 다름]. 자주 헷갈리는 포인트.' },
        { pose: 'lightbulb', text: '[Oracle]: DML 은 기본 [OFF] — 명시적 COMMIT/ROLLBACK 필요.' },
        { pose: 'happy', text: 'Oracle DDL 은 [항상 자동 COMMIT]. 대부분. 이건 설정 X.' },
        { pose: 'think', text: '[SQL Server]: 기본 [ON] — 매 SQL 이 즉시 자동 COMMIT.' },
        { pose: 'lightbulb', text: '명시적 트랜잭션 원하면 BEGIN TRAN. COMMIT/ROLLBACK 으로 묶기.' },
        { pose: 'happy', text: '[핵심 헷갈리는 부분]: Oracle 에서 DDL 실행 시 [직전 DML 도 함께 자동 COMMIT].' },
        { pose: 'think', text: '예: INSERT.; CREATE TABLE.; → INSERT도 영구 반영돼. ROLLBACK으로 되돌릴 수 없어.' },
        ...
        { pose: 'think', text: '헷갈리는 부분: "DDL 후 직전 DML 을 ROLLBACK" → [할 수 없어].' },
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'happy', text: '저장 효율 높아짐 — 거의 모든 경우 VARCHAR2 권장. CHAR 는 고정 길이가 의미 있을 때만.' },
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'think', text: '헷갈리는 부분. 컬럼 [크기 축소]는 [기존 데이터에 영향]. NULL 만 있는 컬럼만 축소 할 수 있어.' },
        { pose: 'lightbulb', text: '예: VARCHAR2(10) 컬럼에 "1234567890" 저장됨 → VARCHAR2(5) 축소 시도 [할 수 없어].' },
        { pose: 'happy', text: 'NUMBER 정밀도 변경도 마찬가지: 데이터가 새 크기 안 들어가면 거부.' },
        { pose: 'think', text: '[삭제 명령] 3종 — DELETE / TRUNCATE / DROP — 자주 헷갈림.' },
        { pose: 'lightbulb', text: '[DELETE]: 행 단위 삭제 (WHERE 할 수 있어). [DML]. ROLLBACK 할 수 있어.' },
        { pose: 'happy', text: '[TRUNCATE]: 전체 행 삭제 (WHERE 조건 없음). [DDL]. ROLLBACK [할 수 없어] + 자동 COMMIT.' },
        { pose: 'think', text: '[DROP TABLE]: 테이블 [자체] 삭제 (구조까지). [DDL]. ROLLBACK으로 되돌릴 수 없어.' },
        { pose: 'lightbulb', text: '헷갈리는 부분. "WHERE 없는 DELETE = DROP" 보기는 [틀림]. DELETE 는 구조 유지, DROP 은 구조도 삭제.' },
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            'ALTER는 테이블 구조를 바꾸고, TRUNCATE는 행을 전부 비우고, DROP은 테이블 자체를 없앱니다. 셋 모두 DDL이지만 지우는 범위가 다르므로 먼저 범위를 구분하면 됩니다.',
        },
        {
          kind: 'table',
          title: 'ALTER 명령 5가지',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'happy', text: '[UNIQUE (UK)]: [유일]만. NULL 허용. 한 테이블 [여러 개] 할 수 있어.' },
        { pose: 'think', text: '[NOT NULL]: NULL을 넣을 수 없어. 단독으로 쓸 수 있어.' },
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '제약조건은 데이터 무결성을 보장하는 가장 기본적인 도구. 자주 나오는 패턴이며, CTAS 의 NOT NULL 만 복제 특성과 VIEW 의 특성도 함께 묶여 나오는 경우가 많습니다.',
        },
        {
          kind: 'table',
          title: '제약조건 6종',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'think', text: '권한을 받은 사람이 다시 다른 사람에게 권한을 줄 수 있는 옵션도 있어.' },
        { pose: 'lightbulb', text: '[WITH GRANT OPTION]: [객체 권한] 받은 user1 이 [다른 사용자에게 재부여 할 수 있어].' },
        { pose: 'happy', text: 'user1 권한 회수 시 → user1 이 user2 에게 준 권한도 [연쇄 회수].' },
        { pose: 'think', text: '[WITH ADMIN OPTION]: [시스템 권한] 받은 user1 이 다른 사용자에게 부여·회수 할 수 있어.' },
