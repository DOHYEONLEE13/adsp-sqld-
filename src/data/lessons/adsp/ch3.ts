...
const ADSP_3_1: Lesson = {
  ...
  topic: 'R 기초와 데이터 마트',
  title: '데이터 마트와 EDA',
  hook: '분석의 80% 는 데이터를 "쓸 수 있게 만드는" 시간이야.',
  estimatedMinutes: 9,
  steps: [
    {
      id: 'adsp-3-1-s1',
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '요약변수와 파생변수를 구분해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '실무에선 모델 만드는 시간보다 데이터 정리 시간이 더 길어. 그 정리 결과물이 "데이터 마트" — 분석 목적에 맞게 다듬어 둔 작은 창고야.',
        },
        {
          kind: 'section',
          title: 'DW vs 데이터 마트',
          body:
            'DW 가 전사 창고라면, 데이터 마트는 "마케팅팀 전용 코너" 처럼 부서·목적별로 뽑아 놓은 작은 저장소야. 더 빠르고, 더 집중되어 있고, 더 만들기 쉬워.',
        },
        {
          kind: 'table',
          title: '요약 vs 파생',
    ...
    // ─── EDA 4원칙 — 1 step → 5 substeps ───
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: 'EDA 원칙이 어떤 상황에 쓰이는지 확인해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            'EDA(Exploratory Data Analysis) 는 튜키(Tukey) 가 정립한 단계. 모델링 전에 데이터의 형태·이상·관계를 파악해.',
        },
        {
          kind: 'callout',
          tone: 'mnemonic',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '저항성(Resistance) 은 "소수의 이상치 때문에 결과가 크게 흔들리지 않는 통계를 쓰자" 는 원칙이야. 평균·표준편차·분산은 한 점이 크게 튀면 통째로 끌려가. 반면 중앙값·IQR·중위절대편차(MAD) 같은 통계는 이상치가 끼어들어도 거의 그대로야.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
          body:
            '학급 시험 점수가 [55 / 58 / 60 / 60 / 62 / 63 / 65 / 68 / 70 / 100(천재 한 명)]. 평균은 66.1 점으로 평범한 학생 누구의 점수도 안 닮음. 중앙값은 62.5 점으로 실제 학급 수준을 잘 대표. 평균이 한 명 때문에 통째로 왜곡된 사례 — 저항성이 떨어지는 통계의 위험을 보여줘.',
        },
        {
          kind: 'callout',
          tone: 'tip',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '잔차해석(Residual) 은 모델 적합 후 잔차(실제 − 예측) 를 들여다보며 모델이 놓친 게 없는지 점검하는 원칙이야. 좋은 모델일수록 잔차가 평균 0 주변에 무작위로 흩어져 있어. 만약 잔차에 어떤 모양·추세가 보이면 그건 모델이 잡아내지 못한 정보가 남아 있다는 신호.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '재표현(Re-expression) 은 변수의 척도(눈금) 를 다른 함수로 바꿔 분석에 더 적합한 형태로 만드는 원칙이야. 한쪽으로 길게 늘어진 분포를 종 모양으로 펴주거나, 분산이 들쑥날쑥한 데이터를 균일하게 만들어 회귀·t검정의 가정을 충족시키기 위해 자주 사용.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '현시성(Revelation) 은 적절한 시각화로 데이터에 숨은 구조·관계·이상을 드러내는 원칙이야. EDA 4원칙 중 가장 자주 마주하는 활동 — 표나 평균값으론 안 보이는 패턴이 그림 한 장에서 단번에 드러나는 경우가 많거든요.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          ...
          title: '복습 체크리스트',
          body:
            '바로 직전 스텝들에서 배운 개념·암기법·함정을 한 번 더 떠올리며 다음 그룹으로 넘어가요. 잘 안 떠오르는 항목이 있으면 이전 스텝으로 돌아가 확인해.',
        },
      ],
    },
    // ─── 결측값 처리 — 1 step → 5 substeps (overview + 4 방식) ───
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '빈칸을 어떻게 다룰지 확인해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '결측값은 데이터 표 안의 "빈칸" 이야. 그냥 비어 있다고 바로 지우면 안 되고, 왜 비었는지와 얼마나 중요한 값인지 먼저 판단해야 해. 그래서 결측 처리는 지우기, 단순히 채우기, 여러 번 추정하기, 모델로 예측하기의 4가지 흐름으로 나뉘어.',
        },
        {
          kind: 'callout',
          tone: 'mnemonic',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '튀는 값을 어떻게 볼지 확인해보자!' },
      ],
      blocks: [
        {
          kind: 'callout',
          ...
          title: '이상값 = 반드시 제거 아님',
          body:
            '오타·기기 오류는 제거. 사기 거래·고가치 고객처럼 진짜 극단값이면 남겨서 분석 대상으로 삼아.',
        },
      ],
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '평균에서 얼마나 멀리 떨어졌는지 확인해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            'ESD(Extreme Studentized Deviate) 는 "평균 ± 3·표준편차" 라는 정규분포 기반의 경계로 이상값을 판정하는 방법이야. 정규분포에선 99.7% 의 데이터가 평균 ±3σ 안에 들어오므로, 이 밖으로 나가면 0.3% 미만의 극단적인 값으로 봐. 종 모양 분포에 가까운 데이터에서 가장 빠르고 단순한 방법.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
        ...
        },
        {
          ...
          title: '함정 — 정규성 깨지면 위험',
          body:
            '소득·매출처럼 한쪽 꼬리가 길게 늘어진 분포(skewed) 는 정규가 아니어서 ESD 가 "정상값을 이상값으로" 또는 그 반대로 오판하기 쉬워. 정규성 미확인이면 IQR 이 안전.',
        },
      ],
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '박스플롯 기준을 확인해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            'IQR(Interquartile Range, 사분위수 범위) 은 Q3(상위 25% 경계) 에서 Q1(하위 25% 경계) 을 뺀 폭이야. 이 폭의 1.5배만큼을 양쪽에 더해 "Q1 − 1.5·IQR 미만" 또는 "Q3 + 1.5·IQR 초과" 를 이상값으로 판정해. 박스플롯의 수염 밖 점들이 바로 이 기준의 이상값.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '표준화 점수 기준을 확인해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            'Z-Score 는 각 데이터 값을 "(값 − 평균) / 표준편차" 로 변환해 평균에서 몇 σ 떨어졌는지를 숫자로 표현하는 방법이야. 변환된 |z| 가 임계값(보통 2 또는 3) 을 넘으면 이상값으로 판정. 임계값을 데이터 특성에 맞게 조절할 수 있다는 게 ESD 보다 유연한 점이야.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
          body:
            '한 학년 모의고사 점수가 평균 70, σ=10 인 학교. 한 학생의 점수가 100점이면 z = (100-70)/10 = 3.0 → 강한 이상값 / 한 학생의 점수가 z = -2.5 → 평균보다 2.5σ 낮음 → 의심 / 보고서 분량 z 가 4 → 평균을 매우 크게 벗어나는 제출물 → 부정 의심 후보. 임계값을 2 로 두면 더 민감하게, 3 으로 두면 보수적으로 잡을 수 있어.',
        },
        {
          kind: 'callout',
          tone: 'tip',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            'DBScan(Density-Based Spatial Clustering of Applications with Noise) 은 본래 군집 알고리즘이지만, "어느 군집에도 속하지 않는 점(noise)" 을 자동으로 분리해주기 때문에 이상값 탐지로도 자주 사용돼. 평균·분산이 아닌 거리·밀도 기반이라 비선형·비정규 분포에서도 강해.',
        },
        {
          kind: 'section',
    ...
    // ─── R 자료구조 — 1 step → 5 substeps (overview + 4 자료구조) ───
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '데이터를 담는 그릇을 구분해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            'R 자료구조는 데이터를 담는 그릇이야. 같은 종류를 한 줄로 담는지, 서로 다른 종류를 섞는지, 표처럼 행과 열이 있는지에 따라 쓰는 그릇이 달라져. 처음에는 "타입이 같은가?" 와 "1차원인가 2차원인가?" 두 질문만 기억하면 돼.',
        },
        {
          kind: 'callout',
          tone: 'mnemonic',
  ...
  steps: [
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '척도가 낮을수록 정보량이 적고, 높을수록 가능한 연산이 자유로워. 명목 < 순서(서열) < 등간 < 비율 위계.',
        },
        {
          kind: 'table',
          headers: ['척도', '예시', '연산'],
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '명목(Nominal) 척도는 카테고리를 구분하는 이름표일 뿐, 그 사이에 어떤 순서·크기·간격도 의미가 없는 척도야. "같다 / 다르다" 의 비교만 통하고, "크다·작다", "차이가 얼마", "두 배" 같은 비교는 모두 무의미해. 척도의 4단계 중 가장 정보량이 적은 단계.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
        ...
        },
        {
          ...
          body:
            '"1=서울, 2=부산, 3=대구" 처럼 숫자 코드가 들어가도 의미가 라벨이면 명목이야. 평균을 내도 "2 = 부산" 이라는 결과가 의미 없으니까요. 학점 A·B·C 처럼 순서가 있으면 다음 단계(순서).',
        },
      ],
    },
    {
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '순서(Ordinal, 서열) 척도는 값들 사이에 순위는 있지만 그 간격이 일정한지는 모르는 척도야. "A > B > C" 라고 줄세우는 건 의미 있지만, "A와 B의 차이가 B와 C의 차이와 같다" 라고 단정할 수는 없어. 차이를 산수(빼기) 로 다루는 건 무의미.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
        ...
        },
        {
          ...
          body:
            '순서 척도에 평균을 내면 의미가 흐릿해져 — 만족도 평균 3.7 이 정확히 어떤 만족 상태인지 단정 못함. 차이가 일정한 의미를 가지면 다음 단계(등간).',
        },
      ],
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'happy', text: '이유 — [0이 절대 0이 아니야]. 섭씨 0°C 가 "온도 없음" 이 아니거든.' },
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '등간(Interval) 척도는 값의 차이(빼기) 가 일정한 의미를 갖는 척도야. "10°C 차이" 가 어디서나 같은 의미야. 하지만 비율(나누기) 은 무의미한데, 그 이유는 "절대 0 (값이 진짜 없는 상태)" 이 정의되지 않았기 때문이야. 0이 그저 "임의로 정한 기준점" 일 뿐.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
          body:
            '섭씨 온도 — 30°C 가 15°C 의 "두 배" 가 아니야. 0°C 도 "온도가 0" 이 아니라 그저 물의 어는점일 뿐 (실제 절대 0 은 -273°C) / 연도 — 2024년이 1012년의 "두 배" 가 아니야. AD/BC 의 0은 임의로 정한 기준점 / IQ 점수 — IQ 100 이 IQ 50 의 "두 배 똑똑" 이 아니야. IQ 0 = "지능 없음" 이 아니거든요. 모두 차이는 의미 있지만 비율은 의미 없는 케이스.',
        },
        {
          kind: 'callout',
          tone: 'warn',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '비율(Ratio) 척도는 절대 0 이 존재해 모든 산술 연산이 의미를 가지는 척도야. "두 배 / 절반" 같은 비율 표현이 명백히 통해. 4단계 척도 중 가장 정보량이 많고 자유롭게 분석할 수 있어.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
    ...
    // ─── 확률분포 — 1 step → 3 substeps (overview + 이산 + 연속) ───
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '두 갈래를 본 뒤 대표 분포를 확인해보자.' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '확률분포는 "어떤 값이 얼마나 자주 나올지"를 그려놓은 지도야. 먼저 결과가 0개, 1개, 2개처럼 셀 수 있는지, 아니면 키·시간·무게처럼 끊기지 않는 값인지부터 나누면 훨씬 쉬워져.',
        },
        {
          kind: 'callout',
          tone: 'tip',
        ...
        {
          kind: 'table',
          title: '처음 보는 사람 기준 예시',
          headers: ['질문', '분포 쪽'],
          rows: [
            ['오늘 결석자가 몇 명?', '이산 — 정수로 셈'],
            ['고객 응답 시간이 몇 초?', '연속 — 구간 위의 값'],
            ['10번 중 성공 몇 번?', '이산 — 이항'],
            ['평균이 기준과 다른가?', '연속 — 정규/t'],
          ],
        },
      ...
      blocks: [
        {
          ...
          title: '"불·효·일·충"',
          body:
            'Unbiased(불편성) · Efficient(효율성) · Consistent(일치성) · Sufficient(충분성). "정규성" 은 포함되지 않아 — 함정.',
        },
      ],
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '평균적으로 참값에 맞는다는 뜻을 확인해보자.' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '불편성(Unbiased) 은 "추정값을 무한 번 반복했을 때 평균이 정확히 진짜 모수가 되는" 성질이야. 한 번의 추정은 빗나갈 수 있어도 체계적인 편향(bias) 이 없다는 뜻이야. 식으로는 E[추정량] = 모수.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '효율성(Efficient) 은 같은 모수를 추정하는 여러 불편추정량 중 분산(또는 MSE) 이 가장 작은 것을 의미해. 평균적으로는 둘 다 정답이라도 한 번의 추정에서 덜 흔들리는 게 더 좋은 추정량 — 매번 더 신뢰할 수 있으니까요.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '표본이 커질수록 정답에 가까워지는지 확인해보자.' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '일치성(Consistent) 은 표본 크기 n 이 커질수록 추정량이 모수에 확률적으로 수렴하는 성질이야. "표본을 무한히 늘리면 결국 정답이 된다" 의 보장. 큰 표본을 가질 수만 있다면 신뢰할 수 있다는 의미라 통계 추론의 핵심 기반.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '충분성(Sufficient) 은 추정량이 표본의 모수 관련 정보를 모두 흡수했음을 의미해. "추정량 값만 알면 충분하다 — 원 표본을 다시 들여다봐도 모수에 대한 더 많은 정보가 안 나온다" 는 보장. 정보 손실 없이 데이터를 압축한 통계량이라는 뜻.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
        ...
        },
        {
          ...
          body:
            '추정량의 4성질은 [불]편성·[효]율성·[일]치성·[충]분성. "정규성" 이 끼어 있으면 그건 함정 선지야.',
        },
      ],
    },
    {
      ...
      blocks: [
        {
          ...
          title: '통계 추론의 마법',
          body:
            '모집단 분포와 무관하게 n 이 커지면 표본 "평균" 의 분포가 정규분포에 가까워진다 — 이것이 CLT. 통계적 추론이 성립하는 토대야.',
        },
        {
          kind: 'callout',
          tone: 'warn',
          title: '흔한 오해',
          body:
            'CLT 는 "개별 관측값이 정규를 따른다" 는 말이 아니야. "표본 평균의 분포" 에 대한 진술이며, 보통 n ≥ 30 이면 정규로 근사해.',
        },
      ],
    },
    {
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '변수가 100개인 고객 데이터 — 그대로 분석하면 모델이 무거워지고 해석도 어려워. "덜 중요한 방향은 버리고 중요한 방향 몇 개로 요약" 하는 것이 PCA. 수십 개 변수를 2~3개 주성분으로 압축해 시각화·모델 속도를 동시에 잡아.',
        },
        {
          kind: 'section',
          title: '핵심 아이디어 — 분산이 큰 방향',
          body:
            '데이터가 가장 넓게 "퍼져 있는" 방향을 1주성분으로, 그다음 퍼진 방향(1주성분과 직교) 을 2주성분으로… 순차로 뽑아. 분산이 클수록 정보량이 많다고 보는 거야. 원 변수들의 선형결합으로 만들어져.',
        },
        {
          kind: 'keypoints',
          title: '적용 절차',
        ...
        },
        {
          ...
          title: '주의점',
          body:
            'PCA 는 **선형** 차원축소라 곡선·비선형 관계는 잡지 못해. 주성분은 원 변수들의 선형결합이라 해석이 어려울 수 있고, 정답 레이블을 고려하지 않는 **비지도** 기법이야.',
        },
      ],
    },
    {
      ...
      blocks: [
        {
          ...
          title: '거리를 지도에 펼친다',
          body:
            '개체들 사이 "얼마나 비슷한가"(유사도) 또는 "얼마나 다른가"(거리) 를 입력받아, 그 관계를 최대한 보존하는 저차원(주로 2D) 좌표에 개체들을 배치하는 기법이야. 도시 간 거리표만으로 세계 지도를 복원해보는 것과 같은 발상.',
        },
        {
          kind: 'table',
          title: '두 가지 방식',
...
const ADSP_3_3: Lesson = {
  ...
  steps: [
    // ─── p-value · 가설검정 5용어 — 1 step → 6 substeps ───
    {
      ...
      dialogue: [
        { pose: 'wave', text: '가설검정은 "주장이 맞는지" 데이터를 보고 판단하는 과정이야.' },
        { pose: 'think', text: '[H₀] · [H₁] · [α] · [p-value] · [1·2종 오류].' },
        { pose: 'idle', text: '먼저 5용어의 역할부터 잡아보자.' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '예를 들어 "새 공부법이 점수를 올린다"는 주장이 있다고 해봅시다. 가설검정은 일단 "차이가 없다"를 기본값으로 두고, 데이터가 그 기본값을 뒤집을 만큼 강한지 확인해.',
        },
        {
          kind: 'table',
          title: '가설검정 5용어 지도',
          headers: ['용어', '처음 보는 사람 기준 의미'],
          rows: [
            ['H₀', '차이 없음 · 효과 없음이라는 기본값'],
            ['H₁', '차이 있음 · 효과 있음이라는 주장'],
            ['α', '잘못 기각해도 감수할 최대 위험'],
            ['p-value', 'H₀가 맞다고 볼 때 이런 결과가 우연히 나올 확률'],
            ['1종/2종 오류', '잘못 기각하거나, 틀린 H₀를 못 버리는 실수'],
          ],
        },
        {
          kind: 'callout',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '기본값으로 두는 가설이 무엇인지 확인해보자.' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '귀무가설(H₀, Null Hypothesis) 은 "차이 없음 · 효과 없음" 을 가정. 검정의 출발점 — 데이터가 H₀ 를 뒤집을 만큼 강한지를 봐.',
        },
        {
          kind: 'section',
          title: '구분 팁',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '우리가 주장하고 싶은 가설을 확인해보자.' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '대립가설(H₁, Alternative Hypothesis) 은 "차이 있음 · 효과 있음" 을 주장. 연구자가 증명하고자 하는 가설로, H₀ 가 기각될 때 채택돼.',
        },
        {
          kind: 'section',
          title: '구분 팁',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: 'p-value를 흔한 오해 없이 확인해보자.' },
      ],
      blocks: [
        {
          kind: 'intro',
        ...
        },
        {
          ...
          body:
            'p-value 는 H₀ 의 진위 확률이 아니야. "H₀ 가 참이라면 이런 데이터가 우연히 나올 확률" 이 정확한 정의.',
        },
      ],
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '1종 오류와 2종 오류를 구분해보자.' },
      ],
      blocks: [
        {
          kind: 'intro',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '일표본(One-sample) t검정은 한 집단의 평균이 알려진 기준값과 같은지를 검증하는 검정이야. H₀: μ = μ₀. 비교 상대가 또 다른 집단이 아니라 "특정 숫자" 라는 게 다른 t검정과의 결정적 차이.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '대응표본(Paired) t검정은 같은 대상에서 두 시점 또는 두 조건을 측정해 차이의 평균이 0인지 검정해. 핵심은 "측정 대상이 같다" 는 것 — 같은 사람·같은 차량·같은 환자가 두 번 측정되어 자연스러운 짝(pair) 을 이뤄. 분석 시엔 차이값(d = 사후 − 사전) 의 평균이 0인지 일표본 t로 풀어.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '독립표본(Two-sample) t검정은 서로 다른 두 집단의 평균을 비교해. 두 그룹의 구성원이 겹치지 않아 짝지을 수 없는 게 대응표본과의 차이. 등분산성이 가정되면 pooled variance, 등분산이 깨졌으면 Welch 의 t를 사용해.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
    ...
    // ─── 회귀 4가정 (선분정독) — 1 step → 5 substeps ───
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '회귀선을 믿어도 되는 조건을 먼저 보자.' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '회귀선은 데이터 위에 그은 "설명선"이야. 그런데 데이터 모양이 직선과 너무 다르거나, 오차가 한쪽으로 몰리거나, 시간 순서대로 서로 영향을 주면 회귀선의 p-value와 해석을 믿기 어려워져. 그래서 4가정을 먼저 확인해.',
        },
        {
          kind: 'table',
          title: '회귀 4가정 한눈에 보기',
          headers: ['가정', '무슨 뜻?', '깨졌을 때 신호'],
          rows: [
            ['선형성', '관계가 직선에 가깝다', '잔차에 곡선 패턴'],
            ['등분산성', '오차 흩어짐이 비슷하다', '잔차가 부채꼴'],
            ['정규성', '잔차가 종 모양이다', 'QQ플롯이 크게 휘어짐'],
            ['독립성', '잔차끼리 영향이 없다', '시계열 자기상관'],
          ],
        },
        {
          kind: 'section',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '선형성(Linearity) 은 "X 가 1단위 증가할 때 Y 도 일정하게 증가/감소한다" 는 직선 관계 가정이야. 회귀의 가장 기본 전제 — 만약 X-Y 가 곡선이거나 어느 구간 후 정체된다면 직선으로 그은 회귀선이 시스템을 잘못 묘사하게 돼.',
        },
        {
          kind: 'section',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '등분산성(Homoscedasticity) 은 X 의 어느 영역에서나 잔차의 흩어짐(분산) 이 비슷하게 일정하다는 가정이야. 깨지면(이분산) 회귀계수의 표준오차 추정이 잘못되어 t·F 검정의 p-value 를 신뢰할 수 없게 돼.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기 + 진단법',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '정규성(Normality) 은 잔차가 평균 0인 정규분포를 따라야 한다는 가정이야. 회귀계수의 신뢰구간·t·F 검정의 p-value 가 정규성 가정 위에서 계산되기 때문에 이 가정이 깨지면 통계적 추론이 흔들려.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기 + 진단법',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '독립성(Independence) 은 잔차들이 서로 상관이 없어야 한다는 가정이야. 한 관측의 잔차가 다른 관측의 잔차에 영향을 주면 안 됨. 시계열·반복측정·군집 데이터에서 자주 깨지며 깨지면 회귀계수 표준오차가 과소/과대 추정되어 검정 신뢰가 떨어져.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기 + 진단법',
      ...
      blocks: [
        {
          ...
          title: 'VIF ≥ 10 이면 의심',
          body:
            '설명변수끼리 상관이 너무 크면 계수 해석이 불안정해져. VIF(Variance Inflation Factor) 가 10 이상이면 다중공선성을 의심해.',
        },
        {
          kind: 'table',
          headers: ['변수 선택법', '설명'],
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '추세(Trend) 는 시계열의 장기적이고 일관된 방향성이야. "수년~수십 년" 단위에서 시계열을 멀찍이 보면 드러나는 큰 흐름 — 우상향, 우하향, 평탄화. 단기 출렁임이나 1년 주기 반복을 모두 걷어낸 뒤 남는 큰 그림이야.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '계절성(Seasonality) 은 시·일·주·월·년 단위처럼 고정된 주기로 정확하게 반복되는 패턴이야. "매년 8월에 늘 같은 모양" 처럼 주기가 일정하다는 게 핵심 — 다음 주기에 어느 정도 비슷한 패턴이 다시 올지 예측이 가능해.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '순환(Cycle) 은 계절성보다 길고 등락이 있지만 주기가 일정하지 않은 변동이야. 다음 등락이 언제 올지 정확히 예측하기 어려운 게 계절성과의 결정적 차이. 보통 수년 단위로 펼쳐지는 큰 사이클이야.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '어떤 변동이 불규칙 성분인지 확인해보자.' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '불규칙(Irregular) 성분은 추세·계절성·순환을 모두 분해해 빼낸 뒤 남는 무작위 변동이야. 일회성 외부 충격, 자연재해, 측정 오차, 알 수 없는 우연 — 시계열 모델이 미리 예측할 수 없는 부분이 모두 여기로 모여.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
        ...
        },
        {
          ...
          body:
            '시계열 4성분은 [추]세·[계]절·[순]환·[불]규칙. "지연(Lag)" 은 ARIMA 모델의 파라미터(AR/MA 의 lag 차수) 이지 분해 성분이 아니야 — 함정 선지로 자주 출제.',
        },
      ],
    },
    {
  ...
  steps: [
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          title: '과적합 vs 과소적합',
          body:
            '훈련 데이터에만 과하게 맞추면 과적합 — 새 데이터에 못 맞춰. 반대로 아예 학습이 덜 되면 과소적합. 균형점을 찾는 것이 모델링의 핵심.',
        },
        {
          kind: 'table',
          title: '분할 방법 4가지',
    ...
    // ─── 앙상블 4종 — 1 step → 5 substeps ───
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '앙상블이 모델을 합치는 방식을 구분해보자!' },
      ],
      blocks: [
        {
          kind: 'callout',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '여러 모델의 투표 방식인지 확인해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            'Voting 은 서로 다른 종류의 분류기·회귀기를 학습시킨 뒤 각자의 예측을 다수결(hard voting) 또는 확률 평균(soft voting) 으로 합치는 가장 단순한 앙상블이야. 친구 여러 명에게 같은 문제를 보여주고 의견을 모은다고 생각하면 직관적이야.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            'Bagging(Bootstrap Aggregating) 은 같은 알고리즘 N 개를 병렬로 학습하되, 각각에게 부트스트랩(복원 허용 무작위 추출) 으로 약간씩 다른 데이터셋을 줘. 결과적으로 N 개 모델의 예측을 평균(다수결) 해서 분산을 줄이고 과적합을 완화해.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            'Boosting 은 약한 학습기(stump 같은 단순 모델) 를 순차로 학습시키며, 매 단계마다 이전 모델이 틀린 샘플에 더 큰 가중치를 두어 점진적으로 오차를 줄여나가는 앙상블이야. 편향(bias) 을 줄이는 게 강점이라 단일 약한 모델로는 못 풀던 문제도 잘 풀어.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            'Stacking 은 여러 베이스 모델(랜덤포레스트·XGBoost·로지스틱 등) 의 예측 결과를 모아 새로운 학습 데이터셋으로 만든 뒤, 그 위에 메타(블렌더) 모델 하나를 더 학습시키는 앙상블이야. "예측의 예측" 이라는 한 층을 더 쌓는 것 — 다양한 모델의 강점을 한 번 더 결합해.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
    ...
    // ─── 연관분석 3 (지신향) — 1 step → 4 substeps ───
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '장바구니 규칙의 3가지 지표를 구분해보자!' },
      ],
      blocks: [
        {
          kind: 'callout',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '두 상품이 함께 나온 비율을 확인해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '지지도(Support) = "A·B 가 함께 등장한 거래 수 ÷ 전체 거래 수" 이야. 발견한 규칙이 얼마나 자주 일어나는지를 측정해. 지지도가 너무 낮으면 어쩌다 한두 번 일어나는 우연일 가능성이 커서, 마이닝 시작 전에 최소 지지도(min support) 기준을 정해 그 이상만 본격 분석해.',
        },
        {
          kind: 'section',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: 'A가 있을 때 B도 나오는 비율을 확인해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '신뢰도(Confidence) = P(B|A) = "A·B 가 함께 등장한 거래 수 ÷ A 가 등장한 거래 수". A 라는 행동이 일어났을 때 B 가 따라올 조건부 확률이야. "맥주를 산 사람들 중 60%가 기저귀도 샀다" 같이 추천·번들링·진열 결정에 직접 쓰여.',
        },
        {
          kind: 'section',
          title: '예시로 — 맥주 → 기저귀',
        ...
        },
        {
          ...
          body:
            '"맥주 → 우유 Confidence 80%" 라도 우유 자체가 거래 90%에 등장하는 흔한 상품이면, 맥주 안 사도 우유 살 사람들이 대부분이라 인과/연관이 약함. 다음 지표인 Lift 가 이걸 보정해.',
        },
      ],
    },
    {
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '향상도(Lift) = 신뢰도 ÷ 전체에서 B 의 등장 비율 = P(B|A) ÷ P(B). 이는 "A 를 봤을 때 B 가 등장할 확률" 이 "A 와 무관하게 B 가 등장할 확률" 의 몇 배인지를 측정해. 연관성의 진짜 강도를 측정하는 핵심 지표 — Confidence 의 함정을 보정해줘.',
        },
        {
          kind: 'section',
          title: '예시로 — Lift 해석',
    ...
    // ─── 군집 알고리즘 — 1 step → 5 substeps (overview + 4 method) ───
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '어떤 기준으로 묶는지 확인해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            '군집은 정답 라벨이 없는 데이터를 비슷한 것끼리 묶는 방법이야. "이 학생이 A반인지 B반인지"를 미리 알려주지 않고, 데이터의 거리·밀도·확률 구조를 보고 자연스러운 그룹을 찾아.',
        },
        {
          kind: 'callout',
          tone: 'mnemonic',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '계층적 군집은 데이터를 점점 합치거나(합병형, agglomerative) 큰 덩어리에서 쪼개며(분할형, divisive) 트리(덴드로그램) 를 형성. K 를 미리 정할 필요 없어. 학교 조직도처럼 "친한 친구 → 같은 반 → 같은 학년 → 학교" 로 점점 묶이는 구조와 동일해.',
        },
        {
          kind: 'section',
          title: '구분 팁',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            'K-means 는 K 개의 중심점을 두고 각 점을 가장 가까운 중심에 할당 → 중심 갱신 → 반복. K 를 미리 정해야 하고, 구형 클러스터에 강함. "친구 그룹 3개 만들어줘" 처럼 K 가 비즈니스 요구로 정해질 때 직관적이야.',
        },
        {
          kind: 'section',
          title: '구분 팁',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            'DBSCAN(Density-Based) 은 밀도가 충분한 영역을 클러스터로, 외곽은 noise(이상값) 로 분류. K 사전 지정 불필요 + 비구형 클러스터 + 이상치 강건. 학급에서 친구가 모여 앉은 자리는 군집, 외딴 학생은 noise 로 자동 분류되는 구조와 동일해.',
        },
        {
          kind: 'section',
          title: '구분 팁',
    ...
    },
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '확률 할당과 격자 배치를 구분해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            'EM(Expectation-Maximization) 은 가우시안 혼합 모델로 각 점이 클러스터에 속할 확률을 학습해 — "이 학생은 A 그룹 70%, B 그룹 30%" 처럼 부드러운 할당. SOM(Self-Organizing Map) 은 신경망 기반 2D 격자 투영으로 고차원 데이터를 격자 좌표로 시각화해.',
        },
        {
          kind: 'section',
          title: '구분 팁',
    ...
    // ─── 분류 평가지표 — 1 step → 5 substeps ───
    {
      ...
      dialogue: [
        ...
        { pose: 'idle', text: '실제값과 예측값의 네 칸을 확인해보자!' },
      ],
      blocks: [
        {
          kind: 'intro',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '정확도(Accuracy) = (TP + TN) ÷ 전체. 시험에서 100문제 중 95개 맞히면 95% 라고 하는 것과 똑같아. 가장 직관적이고 입에 붙는 지표지만, 클래스 불균형(양성과 음성 비율이 크게 차이남) 상황에서는 무력해진다는 결정적 약점이 있어.',
        },
        {
          kind: 'section',
          title: '함정 — 불균형에서의 정확도 거짓말',
          body:
            '학생 합격 예측을 예로 들면 100명 중 진짜 합격은 1명뿐이고 99명은 불합격이야. 모델이 게으르게 "모두 불합격" 이라고만 답해도 99명을 맞히니 정확도는 무려 99%. 하지만 정작 1명의 합격자는 놓쳤어 — 가장 중요한 일을 놓친 모델이야. 희귀 시험 합격·드문 사고 예측처럼 양성이 드문 문제에서 정확도만 보는 건 위험.',
        },
        {
          kind: 'callout',
          tone: 'tip',
          ...
          body:
            '양성:음성 비율이 50:50 ~ 30:70 정도면 정확도가 의미 있어. 1:99 처럼 한쪽이 압도적이면 정밀도·재현율·F1 으로 보조해야 함.',
        },
      ],
    },
    {
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '정밀도(Precision) = TP ÷ (TP + FP). 풀어쓰면 "모델이 양성이라고 외친 횟수 중 진짜 양성이었던 비율". 거짓 양성(FP, 양성이 아닌데 양성이라 외침) 의 비용이 클 때 중요한 지표야.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '재현율(Recall, 민감도 Sensitivity) = TP ÷ (TP + FN). 풀어쓰면 "진짜 양성이 100명이었다면 그 중 모델이 몇 명을 잡아냈는가". 놓침(FN, 양성을 음성으로 오판) 의 비용이 클 때 중요한 지표야.',
        },
        {
          kind: 'section',
          title: '구체 사례로 감잡기',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '"이 환자가 당뇨냐 아니냐" 처럼 **분류** 가 필요한데, 일반 선형회귀는 예측값이 음수나 2 가 나올 수 있어 부적합해. 로지스틱 회귀는 출력을 0~1 사이 확률로 바꿔주는 똑똑한 회귀야.',
        },
        {
          kind: 'callout',
          tone: 'mnemonic',
          title: '핵심 공식 — 시그모이드',
          body:
            'P(y=1 | x) = 1 / (1 + e^−(β₀ + β₁x)). S 자형 곡선으로 확률을 반환 → 보통 0.5 를 기준으로 0 / 1 을 판정. 다중 분류로 확장하면 소프트맥스 회귀야.',
        },
        {
          kind: 'keypoints',
        ...
        },
        {
          ...
          title: '평가·검정 지표',
          body:
            '회귀처럼 R² 를 쓰지 않고 분류 지표(정확도·정밀도·재현율·ROC/AUC) 를 사용해. 계수 검정은 Wald 검정, 모델 비교는 이탈도(Deviance) 또는 AIC.',
        },
      ],
    },
    {
      ...
      blocks: [
        {
          ...
          body:
            '뿌리 노드에서 "나이 > 30?" 같은 질문으로 데이터를 둘(또는 여러 개) 로 나누고, 가지마다 또 질문을 던져. 잎(leaf) 에 도달하면 그게 최종 예측. 규칙이 그대로 보여서 해석이 뛰어난 것이 최대 장점.',
        },
        {
          kind: 'table',
        ...
        },
        {
          ...
          body:
            '나무가 깊어지면 훈련 데이터에 과하게 맞춰져 새 데이터에 흔들려. 사전 가지치기(최대 깊이·최소 샘플 수 제한) 또는 사후 가지치기(일단 크게 키운 뒤 불필요한 가지 제거) 로 해결해.',
        },
        {
          kind: 'keypoints',
          title: '장단점',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '"비슷한 애들이 뭐였는지 보고 따라 한다." 새 데이터가 들어오면 가장 가까운 K개 이웃의 다수결로 범주를 정해. 학습 단계는 데이터를 "저장만" 할 뿐이라서 **게으른 학습기(lazy learner)** 로 불려.',
        },
        {
          kind: 'keypoints',
          title: '핵심 포인트',
        ...
        },
        {
          ...
          title: '표준화는 거의 필수',
          body:
            '키(170cm) 와 체중(65kg) 처럼 스케일이 크게 다르면 큰 값 변수에 거리가 지배당해. min-max 스케일링 또는 Z-점수 표준화로 맞춘 뒤 사용해.',
        },
      ],
    },
    {
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '"베이즈 정리 + 순진한 가정 하나." 각 특징이 서로 **조건부 독립** 이라고 가정하면, 복잡한 결합확률이 "각 특징 확률의 곱" 으로 확 단순해져. 스팸 필터·텍스트 분류의 고전.',
        },
        {
          kind: 'callout',
          tone: 'mnemonic',
          title: '핵심 수식',
          body:
            'P(C | x₁, …, xₙ) ∝ P(C) · Π P(xᵢ | C). 사후확률 ∝ 사전 × 가능도. 클래스마다 이 값을 계산해 가장 큰 값을 골라.',
        },
        {
          kind: 'keypoints',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '두 집단 사이에 선을 긋는다면, **가장 여유롭게** 긋는 게 안전하겠지. SVM 은 그 "여유(margin)" 가 최대가 되는 경계(초평면) 를 찾아. 경계에 가장 가까운 소수 점이 "서포트 벡터" — 이들이 경계를 결정해.',
        },
        {
          kind: 'keypoints',
          title: '핵심 개념',
        ...
        },
        {
          ...
          title: '커널 트릭 — "올려서 풀자"',
          body:
            '선형으로 나눌 수 없는 데이터도 더 높은 차원으로 옮기면 선형 분리가 가능해. RBF(가우시안)·다항·시그모이드 커널이 대표적. 원공간으로 돌아오면 복잡한 곡선 경계처럼 보여.',
        },
        {
          kind: 'callout',
          tone: 'tip',
      ...
      blocks: [
        {
          kind: 'intro',
          body:
            '뇌의 뉴런이 신호를 받아 임계를 넘으면 다음 뉴런에 전달 — 이걸 수식으로 흉내 낸 것이 인공신경망(ANN). 그 층(layer) 을 여러 개 깊게 쌓은 것이 딥러닝(Deep Learning) 이야.',
        },
        {
          kind: 'section',
          title: '기본 구조',
          body:
            '입력층 → (은닉층 × N) → 출력층. 각 뉴런은 "가중합(Σwᵢxᵢ + b) → 활성화 함수 → 출력" 흐름. 학습은 **오차역전파(Backpropagation)** 로 출력 쪽 오차를 뒤에서부터 거꾸로 전달해 가중치를 조정해.',
        },
        {
          kind: 'table',
          title: '대표 활성화 함수',
        ...
        },
        {
          ...
          title: '자주 틀리는 포인트',
          body:
            '은닉층이 1개만 있는 신경망은 얕은(shallow) ANN 이고, "여러 층" 이 깊게 쌓여야 딥러닝으로 봐. 대량 데이터 · GPU 연산 · 역전파 세 박자가 갖춰지며 2010년대 이후 폭발적으로 발전했어.',
        },
      ],
    },
    // ─── 마무리 — ADsP 전 콘텐츠 클리어 시 열림 (admin 검수 모드 우회 가능) ───
    {
      id: 'adsp-3-4-s12-finale',
      title: '마무리',
      group: 'adsp-3-4-s12-finale',
      dialogue: [
        { pose: 'celebrate', text: '와! 여기까지 정말 수고 많았어! 🎉' },
        {
          pose: 'happy',
          text: '데이터 이해부터 분석 기획,\n그리고 정형 데이터 마이닝의\n6대 모델까지 — 하나도 안 빼놓고 해냈네!',
        },
        {
          pose: 'lightbulb',
          text: '시험 직전엔 너무 새 내용을\n파지 말고, [2회차로 복습하고\n개념 공부] 다시 하면 돼!',
        },
        {
          pose: 'wave',
          text: '암기법 (DIKW · 저잔재현 · 시조프인데\n· 도활확최 · 다병순메 · 계중밀확 …)\n다시 한 번 떠올려보자!',
        },
        {
          pose: 'idle',
          text: '시험장에서도 침착하게 — 너는 이미 충분히 준비됐어. 화이팅! 💪',
        },
      ],
      blocks: [
        {
          kind: 'intro',
          body:
            'ADsP 전 과정을 끝까지 완주했어. 데이터의 본질부터 분석 기획·통계·모델링까지 — 데이터 분석 준전문가에게 필요한 핵심을 모두 다뤘어.',
        },
        {
          kind: 'callout',
          tone: 'tip',
          title: '다음 학습 가이드',
          body:
            '시험까지 남은 시간엔 새 내용을 더 채우기보다 [2회차 복습] 으로 약점을 단단하게 다져. 헷갈렸던 step 의 reminder 카드를 다시 한 번 훑고, 오답 복습 모드로 약한 토픽을 보강하면 충분해.',
        },
        {
          kind: 'keypoints',
          title: 'ADsP 핵심 암기법 총정리',
          items: [
            '데이터 본질: 데정지혜 (DIKW) · 공통저변 (DB) · 공표연내 (SECI)',
            '분석 기획: 시조프인데 (거버넌스) · 도활확최 (성숙도) · 원조프 (데이터 거버넌스)',
            '과제 발굴: 탐정해타 (하향식) · 업조기데문아이 (준비도)',
            'EDA·전처리: 저잔재현 (EDA) · 완단다모 (결측) · 벡리매데 (R 자료구조)',
            '통계: 명서등비 (척도) · 추계순불 (추정량) · 불효일충 (좋은 추정량)',
            '모델링: 다병순메 (앙상블) · 계중밀확 (군집) · 지신향 (연관분석)',
          ],
        },
        {
          kind: 'callout',
          ...
          title: '마지막 한 마디',
          body:
            '"꾸준히 한 만큼 시험장에서 발휘된다." 시험 당일까지 충분히 자고, 답안지에 침착하게 옮기고, 모르는 문제는 과감히 넘기고 다시 보면 돼. 화이팅! 🚀',
        },
      ],
    },
  ],
