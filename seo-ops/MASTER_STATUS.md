...
- 공식 기존 글 7URL을 보강해 dad1335로 커밋·푸시했다. CI·Cloudflare 동일 SHA 성공과 7URL의 운영 HTML·번들 반영을 확인했다. 실제 결과는 07-reports/2026-09-12-seo-through-0914-publication.md를 따른다.
...
2026-08-30은 `컴활 1급 2급 차이` 패키지다. 기존 비교 URL에 실기·개별 합격 조건·지원 공고별 선택과 선택 이후 첫 학습 예시를 보강했다. Naver는 친절한 선택 이유·학습 루틴, Threads는 짧은 실행형으로 재작성했다. Instagram은 기존 CSS 이미지 6장을 SUPERSEDED로 보존하고, 캐릭터가 필터 한 개념을 설명하는 인스타툰 6장 프롬프트로 전환했다. 새 이미지 생성·직접 검수·게시는 사용자 단계이며, 8/30 공식 블로그는 이번 9/12 배치에서 공개 확인했고, 외부 원고는 `PENDING`, Instagram은 `NEEDS_REVISION`이다. 8/30은 편성일이고 실제 조사·작성은 9/7이다. 다음 작업 전 `EDITORIAL_RULES.md`를 반드시 읽는다.
