...
- 총 복습 노드는 일반 사용자는 `데이터의 이해` 토픽의 기존 개념 문제를 모두 완료해야 열리며, 프리미엄/어드민은 선행 완료 여부와 관계없이 열람할 수 있게 했다.
- DIKW → 데이터 분류 → 지식 변환 → 저장·분석 기반 → 기업 시스템 흐름을 CSS 지도 도식으로 다시 보여준다.
- 새 문항 ID는 추가하지 않고, 기존 ADsP `데이터의 이해` 2회독 변형 문항 5개를 `quizId`/`extraQuizIds`로 재사용해 Supabase 문제 ID 추가 리스크를 줄였다.
- Supabase seed/upsert는 수행하지 않았다. 기존 문항 재사용이라 신규 서버 문항 반영 대상은 없다.
