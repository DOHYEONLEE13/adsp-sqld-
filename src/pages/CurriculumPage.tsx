...
export default function CurriculumPage({ subject }: Props) {
  ...
  const introDescription = introForSubject(subject, seoDescription);
  ...
  return (
    ...
    >
      <div className="relative z-10 mx-auto max-w-[900px] px-5 pb-16 pt-8 md:px-8 lg:max-w-[1080px] lg:px-12">
        ...
        >
          <h1 className="kr-heading mb-3 text-[28px] leading-[1.15] [word-break:keep-all] md:text-[40px] lg:text-[48px]">
            {headlineForSubject(subject, label)}
          </h1>
          <p
            className={
              usesStudyNoteTheme
          ...
          <div className="flex flex-wrap items-center gap-3 md:gap-4">
            <Stat label="과목" value={curriculum.totalChapters} accent={accent} light={usesStudyNoteTheme} />
            <Stat label="토픽" value={curriculum.totalTopics} accent={accent} light={usesStudyNoteTheme} />
            <Stat
              label={curriculum.isExpansion ? '공개 토픽' : '학습 스텝'}
              value={curriculum.isExpansion ? curriculum.availableTopics : curriculum.totalSteps}
              accent={accent}
              light={usesStudyNoteTheme}
            />
          </div>
        </header>
        ...
        >
          ...
          <p className="kr-body mb-5 text-[13px] leading-[1.65] text-white/75 md:text-[14px]">
            {ctaDescriptionForSubject(subject)}
          </p>
        ...
        <div className="mt-12 border-t border-[#d8e2ce] pt-6 text-center">
          ...
          <p className="kr-body mb-3 text-[12px] text-[#697561]">
            {subject.startsWith('comhwal')
              ...
              : 'QuestDP — 처음 보는 데이터·SQL 용어를 짧은 카드와 문제로 익히는 학습 앱'}
          </p>
          <a
            href="/about"
...
function TopicGroup({
  ...
  if (topic.lessons.length === 0) {
    return (
      <div className={light ? 'rounded-[8px] border border-[#dbe5d2] bg-white p-4 md:p-5' : 'rounded-[14px] border border-cream/10 bg-white/[0.02] p-4 md:p-5'}>
        <h3
          className={light ? 'kr-heading mb-1 text-[15px] text-[#162015] md:text-[16px]' : 'kr-heading mb-1 text-[15px] md:text-[16px]'}
          style={light ? undefined : { color: accent }}
        >
          {topic.topic}
        </h3>
        <p className={light ? 'kr-body text-[12.5px] text-[#67735f]' : 'kr-body text-[12.5px] text-cream/55'}>학습 콘텐츠 준비 중</p>
      </div>
    );
  }
  ...
  return (
    <div
      className={
        light
          ? 'rounded-[8px] border border-[#dbe5d2] bg-white p-4 shadow-[0_10px_35px_-30px_rgba(22,32,21,0.42)] md:p-5'
          : 'rounded-[14px] border border-cream/10 bg-white/[0.02] p-4 md:p-5'
      }
    >
      <header className="mb-3">
        <h3
          className={light ? 'kr-heading mb-0.5 text-[15px] text-[#162015] md:text-[17px]' : 'kr-heading mb-0.5 text-[15px] md:text-[17px]'}
          style={light ? undefined : { color: accent }}
        >
          {topic.topic}
        </h3>
        <div className={light ? 'text-[12px] font-bold text-[#67735f]' : 'text-[10px] uppercase tracking-widest text-cream/50'}>
          Step {totalSteps}개
        </div>
      </header>
          ...
          lesson.steps.map((step) => (
            <li key={step.id}>
              <a
                ...
                className={
                  light
                    ? 'block rounded-[8px] border border-[#e1e8d8] bg-[#f7faf3] px-3 py-2.5 transition hover:border-[#b9c9ad] hover:bg-white'
                    : 'block rounded-[10px] border border-cream/8 px-3 py-2.5 transition hover:border-neon/40 hover:bg-white/[0.04]'
                }
              >
                <div className="flex items-baseline gap-2">
                  ...
                  <span
                    className={
                      light
                        ? 'kr-body text-[13px] font-semibold leading-[1.45] text-[#2d3829] md:text-[13.5px]'
                        : 'kr-body text-[13px] leading-[1.4] text-cream/85 md:text-[13.5px]'
                    }
                  >
                    {step.title}
                  </span>
    ...
    >
      <header className="mb-3 flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <h3
            className={light ? 'kr-heading text-[15px] text-[#162015] md:text-[17px]' : 'kr-heading text-[15px] md:text-[17px]'}
            style={light ? undefined : { color: accent }}
          >
            {topic.topic}
          </h3>
...
function headlineForSubject(subject: SeoCurriculumSubject, label: string): string {
  if (subject === 'adsp') {
    return 'ADsP, 3과목을 같은 방식으로 외우지 마세요';
  }
  if (subject === 'sqld') {
    return 'SQLD, SQL부터 손에 잡히게 보세요';
  }
  return `${label}, 여기서부터 보면 덜 막혀요`;
}

function introForSubject(subject: SeoCurriculumSubject, fallback: string): string {
  if (subject === 'adsp') {
    return 'ADsP는 범위 이름만 보면 전부 암기처럼 보이지만, 실제로는 1·2과목에서 말문을 트고 3과목을 자주 회전시키는 시험입니다. 데이터 이해, 분석 기획, 통계·R·머신러닝을 한 번에 붙잡지 말고 짧은 개념과 바로 푸는 문제로 나눠 보세요.';
  }
  if (subject === 'sqld') {
    return 'SQLD는 “SQL을 많이 본 사람”에게 유리한 시험입니다. 모델링은 큰 틀을 먼저 잡고, SELECT 실행 순서·JOIN·서브쿼리·윈도우 함수처럼 손이 멈추는 구간을 잘게 끊어 반복하는 쪽이 덜 막힙니다.';
  }
  if (subject === 'comhwal-1') {
    return '1급은 범위가 넓어서 처음부터 세 과목을 같은 무게로 들고 가면 금방 지칩니다. 공통 과목인 컴퓨터 일반으로 말문을 트고, 스프레드시트와 데이터베이스를 화면 감각에 맞춰 붙이는 순서로 보세요.';
  }
  if (subject === 'comhwal-2') {
    return '2급은 데이터베이스가 빠지는 대신 컴퓨터 일반과 스프레드시트에서 실수를 줄이는 싸움입니다. 낯선 용어를 먼저 풀어내고, 엑셀 화면에서 바로 떠올릴 수 있는 단어부터 쌓으면 훨씬 덜 막힙니다.';
  }
  if (subject === 'comhwal') {
    return '컴활은 범위표만 보면 딱딱하지만, 실제로는 “컴퓨터 화면에서 자주 만나는 말”을 차례로 익히는 시험입니다. 1급과 2급의 차이를 먼저 확인하고, 지금 공개된 개념 카드부터 부담 없이 시작하세요.';
  }
  return fallback;
}

function ctaDescriptionForSubject(subject: SeoCurriculumSubject): string {
  if (subject === 'adsp') {
    return '커리큘럼을 훑었다면 1과목부터 짧게 시작하세요. 개념을 오래 읽기보다 한 스텝 보고 바로 문제를 풀어야 3과목까지 흐름이 끊기지 않습니다.';
  }
  if (subject === 'sqld') {
    return '범위를 봤다면 바로 SQL 문제로 손을 움직여 보세요. 한 문법을 읽고 끝내기보다, 같은 문법이 선지에서 어떻게 바뀌는지 확인하는 쪽이 점수로 이어집니다.';
  }
  return '커리큘럼을 훑었다면 이제 실제 카드로 넘어가세요. 한 번에 많이 외우기보다, 낯선 단어를 보고 바로 한 문제로 확인하는 흐름이 훨씬 오래 갑니다.';
}
