...
export const GLOSSARY_TERMS: Record<string, { title: string; body: string }> = {
  ...
  키: {
    title: '키',
    body:
      '키는 표에서 원하는 한 행을 찾거나, 다른 표와 연결할 때 쓰는 기준값이야. 학번처럼 한 학생을 찾는 값을 떠올리면 돼.',
  },
  슈퍼키: {
    title: '슈퍼키',
    body:
      '슈퍼키는 한 행을 유일하게 찾을 수 있는 속성 묶음이야. 다만 불필요한 속성이 붙어 있어도 슈퍼키가 될 수 있어.',
  },
  후보키: {
    title: '후보키',
    body:
      '후보키는 한 행을 유일하게 찾을 수 있으면서 더 이상 줄일 수 없는 최소한의 키 후보야.',
  },
  기본키: {
    title: '기본키',
    body:
      '기본키는 후보키 중에서 대표로 선택한 키야. 보통 PK라고 부르고, 한 행의 대표 이름표 역할을 해.',
  },
  대체키: {
    title: '대체키',
    body:
      '대체키는 후보키였지만 기본키로 선택되지 않은 나머지 후보키야. 대표는 아니지만 유일하게 찾을 수 있는 값이야.',
  },
  외래키: {
    title: '외래키',
    body:
      '외래키는 다른 표의 대표 키를 가져와 두 표를 연결하는 키야. 보통 FK라고 부른다.',
  },
  PK: {
    title: 'PK',
    body:
  ...
  NULL: {
    title: 'NULL',
    body:
      'NULL은 값이 0이라는 뜻이 아니라 아직 값이 없거나 알 수 없다는 뜻이야. PK에는 NULL이 들어갈 수 없어.',
  },
  UNIQUE: {
    title: 'UNIQUE',
    body:
      'UNIQUE는 같은 값이 두 번 나오지 않게 막는 제약이야. 대표 이름표인 PK와 달리 중복 방지가 핵심이고, SQLD/Oracle 기준으로 NULL은 허용될 수 있어.',
  },
  FK: {
    title: 'FK',
    body:
  ...
  정규화: {
    title: '정규화',
    body:
      '정규화는 한 표에 섞인 정보를 성격별로 나누어 중복을 줄이는 과정이야. 중복을 줄이면 삽입·삭제·갱신 이상을 예방하기 쉬워져.',
  },
  '이상 현상': {
    title: '이상 현상',
    body:
      '이상 현상은 중복 때문에 데이터를 넣거나, 지우거나, 고칠 때 생기는 부작용이야. 정규화를 배우는 대표적인 이유야.',
  },
  '개체 무결성': {
    title: '개체 무결성',
    body:
