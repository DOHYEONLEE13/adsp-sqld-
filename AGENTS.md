<!-- ── AI Agent Instruction 파일 (2026-05 신설 동기화 메타) ─────────────
  이 파일은 Codex (OpenAI Codex CLI / Codex Cloud) 용 — `codex` 가 프로젝트
  루트에서 자동 발견 (https://agents.md/ 표준).
  Claude Code (`claude code` CLI) 용으로는 **`CLAUDE.md`** 가 동일 내용으로 별도 존재.
  **한쪽 수정 시 반드시 다른 쪽도 동기화** (sync 스크립트 미사용 — 수동).
  drift 발생 시 마지막 commit 의 diff 로 어느 쪽이 오래됐는지 판단.
─────────────────────────────────────────────────────────────── -->

# AGENTS.md — QuestDP 세션 인계 노트

> **이 파일의 역할.** 새 세션의 Codex (또는 다른 AI agent) 가 "어제까지 어디까지 해놨고, 다음에 뭘 이어야 하며, 건드리면 안 되는 규칙이 뭔지" 를 5분 안에 파악하기 위한 살아있는 문서. 구현이 바뀌면 여기도 같이 업데이트.
>
...
### 🟦 P3 — 문제은행 비공개화 / 서버 발급 완전 전환 (나중에)

**절대 원칙: 문제은행 원본은 삭제하지 않는다.** 여기서 말하는 "문제은행 제거"는 운영자/제작자가 수정하는 원본 데이터 삭제가 아니라, 일반 사용자 브라우저 번들에서 **정답·해설 포함 전체 문제은행 사본을 제외**한다는 뜻이다.

현재 판단 (2026-05-22): 아직 바로 진행하면 위험하다. `GamePage → session.ts → ALL_QUESTIONS`, `data/lessons/index.ts → ALL_QUESTIONS` 연결이 남아 있고, 레슨/복습/약점/통계/북마크/SEO 정적 페이지가 문제 메타데이터에 강하게 묶여 있다. 지금 무리하게 걷어내면 보안 강화보다 기능 파손 위험이 더 크다.

나중에 진행할 때의 안전한 순서:

1. v2 서버 문제 발급/제출/쿼터/XP/쿠폰/프리미엄이 운영에서 안정적으로 동작하는지 먼저 검증한다.
2. `createSession`, `createMockExamSession`, `createReviewSession`, `createReviewFromIds`, 레슨 단일 문제 진입이 클라이언트 문제 선택이 아니라 서버 발급 결과만 사용하게 바꾼다.
3. 통계/약점/북마크/로드맵 카운트에는 정답 없는 `question_meta` 전용 RPC 또는 view 를 제공한다. 일반 클라이언트는 `answer_index`, 정답 해설, 전체 문제 원문을 미리 받지 않는다.
4. 관리자 대시보드 또는 운영 스크립트에서는 원본 문제를 계속 조회·수정·검증·시드할 수 있어야 한다. `src/data/questions`, `question-bank`, Supabase `questions` 중 어떤 원천을 쓰든 삭제하지 않는다.
5. SEO/정적 페이지는 공개해도 되는 범위만 별도로 남긴다. `/quiz/*` 대량 공개나 정답 포함 HTML 공개는 별도 검토 전까지 하지 않는다.
6. 전환 후 `npm run typecheck`, `npm test -- --run`, `npm run build`, 주요 E2E, 그리고 `dist/assets` grep 으로 정답 포함 문제은행이 일반 번들에 남지 않았는지 확인한다.

중단 조건:
- 레슨 문제 풀이, 복습 세션, 약점 분석, 북마크, 통계 중 하나라도 기존 사용자 흐름이 깨질 가능성이 보이면 중단한다.
- 운영자가 직접 문제를 수정할 수 없는 구조가 되면 중단한다.
- 서버 RPC 없이 클라이언트가 다시 전체 문제를 고르는 fallback 을 만들면 중단한다.
