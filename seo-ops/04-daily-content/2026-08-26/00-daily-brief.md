...
DEMAND EVIDENCE: 2026-08-26 Naver Blog 결과에서 비전공자 2주·3주·4주 플랜, 한 달 독학, 인강과 문제풀이를 연결한 글이 반복 노출됐다. QuestDP의 기존 비전공자 가이드도 결과에서 확인됐다.
PRIORITY: P1 / 92
CURRENT QUESTDP COVERAGE: STRONG BUT MISALIGNED — 기존 URL이 노출되고 내용도 충분하지만, 시험을 시작하지 않은 독자에게 과목 내부 개념을 너무 많이 설명해 첫 공부 순서가 흐려졌다.
TOP SERP PATTERNS: 비전공자 기간별 플랜, 3과목 시간 배분, 독학과 인강 선택, 문제집·기출 회독, 합격 후기형 제목
CONTENT GAP: 세 과목을 같은 비중으로 나누지 않는 이유와, 통계·R이 처음인 사람이 어디에서 오래 걸리는지를 개념 선행 지식 없이 설명하는 입문 가이드가 필요하다.
ACTION: UPDATE EXISTING
CANNIBALIZATION: 새 URL을 만들지 않고 `/blog/adsp-비전공자-가이드/`를 `ADsP 공부법` 필러로 강화한다. 2주 intent와 독학·인강 비교 intent는 기존 글로 내부 링크한다.
